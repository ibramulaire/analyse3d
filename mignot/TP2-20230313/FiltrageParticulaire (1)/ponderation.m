function Particules = ponderation(Particules,Config,I)
	...
	if Config.fonction_de_vraisemblance<2
      ...
      % fenetre de 5x5 autour de la ,particule  
			...
			%recupere les pixels et faire la moyenne
			Icrop=I(w1:w2,h1:h2,:);
			moy= mean(Icrop(:));
			%celui avec le plus de tache noir au alentour aura la plus grande probabilité
	...
	elseif Config.fonction_de_vraisemblance<4
		...
		for p=1:Config.nombre_de_particule
			...
			Icrop=I(w1:w2,h1:h2,:);
      %selui avec le soisin le plus sombre aura la probabilité la plus grande
		...
		somme=0;
		for p=1:Config.nombre_de_particule
     % la particule la plus sombre aura la probabilité la plus grande
